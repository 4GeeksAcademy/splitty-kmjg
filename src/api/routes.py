...
def create_expense(group_id):
    ...
    try:
        ...
        for p_id in participants:
            part = ExpenseParticipant(expense_id=new_expense.id, user_id=p_id)
            db.session.add(part)
        db.session.commit()
        return jsonify({"message": "Gasto creado", "expense": new_expense.serialize()}), 201
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def get_group_expenses(group_id):
    ...
    return jsonify({"expenses": [e.serialize() for e in expenses]}), 200